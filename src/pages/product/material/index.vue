<template>
    <div>
            ...
            <div class="padding-add">
                <el-form ref="artFrom" :model="artFrom" label-width="80px" label-position="right" class="tabform"
                    ...
                    <el-form-item label="规格搜索：">
                        <el-input clearable v-model="artFrom.rule_name" placeholder="请输入规格名称"
        ...
        <el-card :bordered="false" shadow="never" class="ivu-mt mt16">
            <el-button v-auth="['product-rule-save']" type="primary" @click="addAttr">添加材质</el-button>
            ...
            <el-table ref="table" :data="tableList" v-loading="loading" highlight-current-row :row-key="getRowKey"
                ...
                <el-table-column label="材质名称" min-width="130">
                    <template slot-scope="scope">
                        <span>{{ scope.row.name }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="材质图片" min-width="130">
                ...
                <!-- <el-table-column label="商品属性" min-width="130">
                    <template slot-scope="scope">
                        <span v-for="(item, index) in scope.row.attr_value" :key="index" v-text="item"
                ...
                <el-table-column label="操作" fixed="right" width="120">
                    <template slot-scope="scope">
                        ...
                        <a @click="del(scope.row, '删除材质', scope.$index)">删除</a>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <add-attr ref="addattr" @upd="getDataList"></add-attr>
        <el-dialog :visible.sync="pictureModal" width="950px" title="上传材质图" :close-on-click-modal="false">
            <uploadPictures @getPic="getPic" @getPicD="getPicD" :gridBtn="gridBtn" :gridPic="gridPic" v-if="pictureModal">
            </uploadPictures>
        </el-dialog>
    </div>
</template>
...
import addAttr from './add.vue';
import { get_material, del_material } from '@/api/product';
export default {
    name: 'productAttr',
    components: { addAttr },
    data() {
