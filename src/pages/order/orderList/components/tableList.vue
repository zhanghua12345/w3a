<template>
  <div>
    ...
    <el-table :data="orderList" ref="table" v-loading="loading" empty-text="暂无数据" @select="handleSelectRow"
      @select-all="handleSelectRow" class="orderData mt14">
      ...
      <el-table-column label="商品信息" min-width="250">
        <template slot-scope="scope">
          <div class="tab" v-for="(item, i) in scope.row._info" :key="i">
            <img v-lazy="item.cart_info.productInfo.attrInfo
      ...
      <el-table-column label="操作" fixed="right" width="130">
        <template slot-scope="scope">
          ...
          <a @click="exp(scope.row)">导出</a>
          <el-divider direction="vertical"></el-divider>
          <!-- <a @click="sendOrder(scope.row)" v-if="(scope.row.status === 4 || scope.row._status === 2 || scope.row._status === 8) &&
          ...
          <a @click="bindWrite(scope.row)" v-if="scope.row.shipping_type == 2 &&
            scope.row.status == 0 &&
            scope.row.paid == 1 &&
            scope.row.refund_status === 0
            ">立即核销</a>
          <el-divider direction="vertical" v-if="(scope.row._status === 2 && scope.row.shipping_type === 1 && scope.row.pinkStatus === 2) ||
            (scope.row.split.length && scope.row.is_del !== 1)
            " />
          <el-divider direction="vertical" v-if="scope.row.refund_type !== 4 &&
          ...
          <template>
            <el-dropdown size="small" @command="changeMenu(scope.row, $event)" :transfer="true">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="1" v-show="scope.row._status === 1 &&
                  scope.row.paid === 0 &&
                ...
                <el-dropdown-item command="11"
                  v-show="scope.row._status >= 3 && scope.row.express_dump">电子面单打印</el-dropdown-item>
                <!-- <el-dropdown-item command="10" v-show="scope.row._status >= 2">小票打印</el-dropdown-item> -->
                <el-dropdown-item command="4" v-show="scope.row._status !== 1 ||
                  (scope.row._status === 3 &&
                ...
                <!-- <el-dropdown-item command="14">导出</el-dropdown-item> -->
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </template>
    ...
    <order-send ref="send" :orderId="orderId" :status="status" :pay_type="pay_type" :virtual_type="virtual_type"
      @submitFail="submitFail" @clearId="() => {
        ...
        "></order-send>
    <!--    -->
    <el-dialog :visible.sync="modals2" title="订单核销" class="paymentFooter" :show-close="true" width="540px"
      @closed="changeModal">
      <el-form ref="writeOffFrom" :model="writeOffFrom" :rules="writeOffRules" label-width="80px" label-position="right"
        class="tabform" @submit.native.prevent>
        <el-form-item prop="code" label="核销码：">
          <el-input style="width: 414px" type="text" placeholder="请输入12位核销码" v-model.number="writeOffFrom.code" />
        </el-form-item>
      </el-form>
      <div slot="footer">
    ...
    <el-dialog :visible.sync="batchShipmentModal" title="批量发货" class="paymentFooter" :show-close="true" width="540px"
      @closed="changeModal">
    ...
    changeMenu(row, name) {
      ...
      switch (name) {
        ...
        case '14':
          ...
          // let baseURL = Setting.apiBaseURL
          // let _url = `${baseURL}/order/export?id=${row.id}`
          // let headers = {
          ...
          exp_order(row.id).then(response => {
            ...
            link.href = url
            link.setAttribute('download', `${row.order_id}.xlsx`)
            ...
            console.log(err, '失败');
            // location.href = err
            let url = window.URL.createObjectURL(new Blob([err]))
            let link = document.createElement('a')
            link.style.display = 'none'
            link.href = url
            link.setAttribute('download', `${row.order_id}.xlsx`)
