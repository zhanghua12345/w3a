  ...
  <div>
    <el-card :bordered="false" shadow="never" class="ivu-mb-16" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form ref="formValidate" :model="formValidate" label-width="80px" label-position="right" @submit.native.prevent
          inline>
          ...
          <el-form-item label="搜索：" label-for="status2">
            <el-input clearable placeholder="请输入姓名或者账号" v-model="formValidate.name" class="form_content_width" />
          </el-form-item>
          <el-form-item>
    ...
    <el-card :bordered="false" shadow="never" class="ivu-mt">
      <el-button v-auth="['setting-system_admin-add']" type="primary" @click="add">添加管理员</el-button>
      <el-table :data="list" class="mt14" no-userFrom-text="暂无数据" no-filtered-userFrom-text="暂无筛选结果" v-loading="loading"
        ...
        <!-- <el-table-column label="分享图片" min-width="100">
          <template slot-scope="scope">
            <div class="tabBox_img" v-viewer>
              <img v-lazy="scope.row.qrcode" />
            </div>
          </template>
        </el-table-column> -->
        ...
        <el-table-column label="操作" fixed="right" min-width="100">
          <template slot-scope="scope">
            <!-- <a @click="createQr(scope.row)">生成二维码</a> -->
...
import { adminListApi, adminFromApi, adminEditFromApi, setShowApi, get_qr } from '@/api/systemAdmin';
...
export default {
  ...
  methods: {
    ...
    createQr(row) {
      ...
      let rolesNames = row.roles.split(',')
      adminEditFromApi(row.id).then(res => {
        let rules = res.data.rules
        let options = rules.find(item => item.field == 'roles').options
        console.log(options, 'options');
        for (var i = 0; i < rolesNames.length; i++) {
          for (var j = 0; j < options.length; j++) {
            if (rolesNames[i] == options[j].label) {
              form.roles.push(options[j].value)
            }
          }
        }
        get_qr({
